...
raup_crick_stegen <- function(spXsite, reps) {
	...
	alpha_table<-data.frame(matrix(nrow=sum(dim(spXsite)[1]), ncol=2))
...
test_reps <- 15 
system.time({test_res = raup_crick_stegen(test_spXsite, test_reps)})

test_spXsite <- real_spXsite[1:8,]
test_reps <- 30
system.time({test_res = raup_crick_stegen(test_spXsite, test_reps)})
...
write.table(test_res, file = test_out, sep = "\t")
